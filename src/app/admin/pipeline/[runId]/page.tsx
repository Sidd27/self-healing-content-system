...
export default function PipelineRunPage() {
  ...
  if (!run) {
    return <p className="text-muted-foreground text-sm">Loading pipeline run...</p>
  }
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {run.proposedTopics.map((topic) => (
              <div key={topic.id} className="border rounded px-3 py-2 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-medium">{topic.name}</p>
                  <p className="text-xs text-muted-foreground truncate">{topic.description}</p>
                </div>
                <Badge variant="outline" className="shrink-0 text-xs">{topic.status}</Badge>
              </div>
            ))}
          </CardContent>
        </Card>
