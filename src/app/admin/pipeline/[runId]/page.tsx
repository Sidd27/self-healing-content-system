  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {STAGE_ORDER.map((name, index) => {
            ...
            const status: StageStatus = stage?.status ?? (TERMINAL_STATUSES.includes(run.status) ? 'skipped' : 'pending');
            return (
              <div
                key={name}
      ...
      {run.driftItems.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Drift Items ({run.driftItems.length})</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
                  ...
                  {pending && (
                    <div className="flex gap-2 pt-1">
                      <Button
                        size="sm"
                        disabled={busy}
                        onClick={() => reviewDrift(entry.item.id, 'approve')}
                      >
                        Approve
                      </Button>
                      <Button
                  ...
                  {pending && (
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        disabled={busy}
                        onClick={() => reviewTopic(topic.id, 'approve')}
                      >
                        Approve
                      </Button>
                      <Button
                        ...
                        variant="outline"
                        disabled={busy}
                        onClick={() => reviewTopic(topic.id, 'reject')}
                      >
                        Reject
                      </Button>
                    </div>
