...
export default function TopicLearnerPage() {
  ...
  if (!unit) return <p className="text-sm text-muted-foreground">No content yet — run the pipeline to generate learning content.</p>
  ...
  if (quiz.step === 'lesson') {
    return (
      <div className="space-y-6 max-w-2xl">
        <div>
          <p className="text-xs uppercase tracking-wide text-muted-foreground font-medium mb-1">Step 1 — Lesson</p>
    ...
    <div className="space-y-6 max-w-2xl">
      <div>
        ...
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold">{unit.topicName}</h1>
          <span className="text-sm text-muted-foreground">{questionIndex + 1} / {totalQuestions}</span>
        </div>
      </div>
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isAnswered && i === current.correctIndex && 'bg-green-50 border-green-500 text-green-800',
                  isAnswered && i === selected && i !== current.correctIndex && 'bg-red-50 border-red-400 text-red-800',
      ...
      <div className="flex gap-3">
        ...
        {isAnswered && isLast && (
          <Button onClick={restart} variant="outline">Start Over</Button>
        )}
      </div>
    </div>
  )
